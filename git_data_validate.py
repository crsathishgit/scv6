...
def preprocess_segregate_error_rows(columns):
    ...
    with open('./csvsource/weatherSubset.csv', 'r') as csv_file:
...
def do_validation():
    print("===================>>>>>>")
    preprocess_segregate_error_rows(weather_csv_schema.getColumnsCount())
    print("<<<<<<===================")
    # read the data
    data = pd.read_csv('./csvproduced/clean_rows_preprocessed.csv')
    ...
    # save data
    pd.DataFrame({'col':errors}).to_csv('./csvproduced/git_errors.csv')
    print("Number of clean rows:")
    print(len(data_clean))

    print("Number of dirty rows:")
    print(len(errors_index_rows))
    data_clean.to_csv('./csvproduced/git_clean_data.csv')
